  ...
  signals.async_wait([](const boost::system::error_code &ec, int signum) {
    if (ec) {
      BOOST_LOG_TRIVIAL(error)
          << "error ecountered when waiting for signals. code: " << ec.value()
          << "(" << ec.message() << ")";
    } else {
...
void prepare_accept_remote_connection(const RuntimeConfig &config) {
  BOOST_LOG_TRIVIAL(info) << "listening on " << config.listen_address_ << ":"
                          << config.listen_port_;
  io::ip::tcp::resolver resolver(ioservice_);
  io::ip::tcp::resolver::query query(config.listen_address_,
                                     std::to_string(config.listen_port_));
...
void accept_remote_connection(const RuntimeConfig &config) {
  ...
  auto session =
      std::make_shared<ClientEngine::ClientSession>(std::move(remoteSocket));
...
void play(int pos) { Player::play(); }

void next() { Player::next(); }

void prev() { Player::prev(); }

void stop() { Player::stop(); }

void add(const std::string &uri) { Player::playlist().add(uri); }

void enumeratePlaylist(EnumPlaylistFn fn) { Player::playlist().enumerate(fn); }
...
const Player::Status &status() { return Player::status(); }

fs::path workdir() {
  boost::system::error_code ec;
  if (!fs::exists(RuntimeConfig::workdir_, ec)) {
    BOOST_LOG_TRIVIAL(debug) << "workdir " << RuntimeConfig::workdir_
                             << " not found (" << ec.message()
                             << "). Creating it...";
    ec.clear();
    ...
    if (ec) {
      BOOST_LOG_TRIVIAL(error) << "cannot create working directory "
                               << RuntimeConfig::workdir_
                               << " cause: " << ec.message();
      exit(EXIT_FAILURE);
    }
  }
